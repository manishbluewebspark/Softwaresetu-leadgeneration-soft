...
const Hr = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.log("Data fetched successfully:", {
        ...
        offerLetters: offerLettersRes.data.length
      });
  ...
  const viewPDF = async (htmlString, letterId) => {
    setPdfLoadingId(letterId);
    try {
      const response = await axios.post(
        `${apiUrl}/offerletters/generate-pdf`,
        { htmlContent: htmlString },
      ...
      const link = document.createElement('a');
      link.href = url;
      // Generate filename based on employee name if available
      const letter = offerLetters.find(l => l.id === letterId);
      const employeeName = letter?.employee_name || letter?.name || 'offer-letter';
      const fileName = `${employeeName.replace(/\s+/g, '-')}-offer-letter.pdf`;
  ...
  // Handle modal success - यहाँ हम fetchData() को फिर से call करेंगे
  const handleModalSuccess = async () => {
    console.log("Offer letter generated successfully, fetching updated data...");
    
    // थोड़ा delay देकर फिर data fetch करें
    setTimeout(async () => {
      await fetchData();
      toast.success("Offer Letter Generated Successfully! List updated.");
    }, 300);
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 border border-gray-200">
          ...
          {loading ? (
            <div className="flex justify-center items-center py-12">
              <FaSpinner className="h-8 w-8 text-blue-600 animate-spin" />
              <span className="ml-3 text-gray-600">Loading offer letters...</span>
            </div>
          ) : offerLetters.length === 0 ? (
            <div className="text-center py-12">
              <FaFileAlt className="h-16 w-16 text-gray-300 mx-auto mb-4" />
                      ...
                      <tr key={letter.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="text-sm font-medium text-gray-900">
                                {letter.employee_name || letter.name || "N/A"}
                              </div>
                              <div className="text-xs text-gray-500">ID: {letter.id}</div>
                            </div>
                          </div>
                        ...
                        {/* Position */}
                        <td className="px-4 sm:px-6 py-4">
                          <div className="text-sm text-gray-900">{letter.position || "N/A"}</div>
                        </td>

                        {/* Joining Date */}
                        <td className="px-4 sm:px-6 py-4">
                          <div className="text-sm bg-blue-50 px-2 py-1 rounded-md inline-block">
                            {formatDate(letter.joining_date || letter.joiningDate)}
                          </div>
                        </td>
                        ...
                        <td className="px-4 sm:px-6 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            {/* View PDF */}
                            <button
                              onClick={() => viewPDF(letter.final_html || letter.html, letter.id)}
                              disabled={pdfLoadingId === letter.id || (!letter.final_html && !letter.html)}
                              className="inline-flex items-center px-3 py-2 border border-blue-300 text-sm font-medium rounded-lg text-blue-700 bg-blue-50 hover:bg-blue-100 disabled:opacity-50 disabled:cursor-not-allowed"
                              title={(!letter.final_html && !letter.html) ? "No PDF available" : "Download PDF"}
                            >
                              {pdfLoadingId === letter.id ? (
                                <FaSpinner className="h-3 w-3 mr-1 animate-spin" />
                              ) : (
                                <FaEye className="h-3 w-3 mr-1" />
                              )}
